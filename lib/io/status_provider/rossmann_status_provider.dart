...
class RossmannStatusProvider implements FilmDevelopmentStatusProvider {
  static const String API_ENDPOINT = "https://service.fujifilm-imaging.eu/a/rossmannb2";

  //RegEx for Dates in format 01.04.2020 bzw. dd.MM.yyyy
  final RegExp dateRegExp = new RegExp(r"\d{2}\.\d{2}\.\d{4}", caseSensitive: false);
  ...
  final Map<String, FilmDevelopmentStatusSummary> markerTextToStatusSummary = {
    "Ihr Auftrag ist fertiggestellt(noch nicht geliefert)": FilmDevelopmentStatusSummary.SHIPPING,
    "Ihr Auftrag ist fertiggestellt und hat unser Labor am": FilmDevelopmentStatusSummary.SHIPPING,
    "in unserem Labor eingegangen und wird derzeit bearbeitet.": FilmDevelopmentStatusSummary.PROCESSING
  };
  ...
  Future<FilmDevelopmentStatus> obtainDevelopmentStatusForFilmOrder(FilmDevelopmentOrder film) async {
    ...
    //perform request
    http.Response httResponse = await http
        .post(API_ENDPOINT, body: {"AUFNR_A": orderNumber, "FIRMA": firma, "HDNR": htNumber, "x": "0", "y": "0"});
    //Evaluate state of Order
    String stateEvaluation = parse(httResponse.body).querySelector("tr td div").text.trim();
    //Obtain StatusSummary to decide which path to follow
    FilmDevelopmentStatusSummary statusSummary = getFilmDevelopmentStatusSummaryFromText(stateEvaluation);

    switch (statusSummary) {
      case FilmDevelopmentStatusSummary.UNKNOWN_ERROR:
        String errorCaseData = parse(httResponse.body).querySelector("tr td div").text.trim();
        return new FilmDevelopmentStatus(DateTime.now(), statusSummary, 0.0, errorCaseData);

      case FilmDevelopmentStatusSummary.PROCESSING:
        String processingCaseData = parse(httResponse.body).querySelector("tr td div").text.trim();
        DateTime statusSummaryDate = Jiffy(dateRegExp.firstMatch(processingCaseData).group(0), "dd.MM.yyyy").dateTime;
        return new FilmDevelopmentStatus(statusSummaryDate, statusSummary, 0.0, processingCaseData);
      ...
      case FilmDevelopmentStatusSummary.SHIPPING:
        ...
        String shippingCaseStatusLine = parse(httResponse.body).querySelector("tr td div").text.trim();
        shippingCaseStatusLine = shippingCaseStatusLine.replaceAll("\n", "");
        //obtainPrice
        if (priceRegExp.hasMatch(shippingCaseStatusLine)) {
          String priceAsString = priceRegExp.firstMatch(shippingCaseStatusLine).group(0);
        ...
        List<Element> shippingTable = parse(httResponse.body).querySelectorAll("tr td");
        ...
        for (Element el in shippingTable) {
          ...
          if (dateRegExp.hasMatch(contentOfTableRow)) {
            statusSummaryDate = Jiffy(dateRegExp.firstMatch(contentOfTableRow).group(0), "dd.MM.yyyy").dateTime;
          }
        }
        return new FilmDevelopmentStatus(statusSummaryDate, statusSummary, price, shippingCaseStatusLine);
  ...
  FilmDevelopmentStatusSummary getFilmDevelopmentStatusSummaryFromText(String text) {
