...
class PhotoPrintitStatusProvider implements FilmDevelopmentStatusProvider {
  static const String API_ENDPOINT = "https://spot.photoprintit.com/spotapi/orderInfo/order";
  static const String SUMMARY_KEY = "summaryStateText";
  static const String SUMMARY_DATE_KEY = "summaryDate";
  String config;

  PhotoPrintitStatusProvider(this.config);
  ...
  Future<FilmDevelopmentStatus> obtainDevelopmentStatusForFilmOrder(FilmDevelopmentOrder film) async {
    ...
    String queryURL = '$API_ENDPOINT?config=$config&fullOrderId=$storeId-$orderNumber';
    ...
    if (httResponse.statusCode == 200) {
      ...
      FilmDevelopmentStatusSummary statusSummary =
          getFilmDevelopmentStatusSummaryFromText(jsonResponse['summaryStateCode']);
      int price = jsonResponse['summaryPrice'];
      //print(jsonResponse);
      return FilmDevelopmentStatus(DateTime.parse(jsonResponse['resultDateTime']), statusSummary,
          price.toDouble() / 100, jsonResponse['summaryStateText']);
    } else {
